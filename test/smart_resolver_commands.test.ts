...
/**
 * Tests that detail and impl use the smart resolver
...
describe('detail: smart resolver integration', () => {
  let tmpDir: string;
  ...
  it('resolves unqualified name via source file and returns class detail', async () => {
    ...
    try {
      const { run } = await import('../src/commands/detail.js');
  ...
  it('suggests refresh when targeted scan does not find the class', async () => {
    ...
    expect(stderrMessages.some(m => m.includes('refresh'))).toBe(true);
    expect(stderrMessages.every(m => !m.includes('[smart_resolver]'))).toBe(true);
  });
});

describe('impl: smart resolver integration', () => {
  let tmpDir: string;
  ...
  it('resolves unqualified interface name and returns implementations', async () => {
    ...
    try {
      const { run } = await import('../src/commands/impl.js');
